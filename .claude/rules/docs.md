...
- ガイドの節は `docs/guides/<file>.md「<見出し>」` の形で指す。見出しを変えたら `git grep` で指している側も直す (ADR-0051)
...
- ガイドを `@` で import せず、「読む」とも書かない。import は `paths` があっても起動時に常時読み込まれ、案内は読まれない。規範は rules 自身が持つ (ADR-0051)
