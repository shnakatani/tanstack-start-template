...
- ADR にするのは構造・主要な品質特性・戻しにくさに効く選択だけにする。手順や規範の表は ADR ではなくガイドに置く (ADR-0000)
...
- ガイドの節は `docs/guides/<file>.md「<見出し>」` の形で指す。見出しを変えたら `git grep` で指している側も直す (ADR-0005)
...
- 実測値の詳細、選択肢の比較と却下理由、改訂履歴、出典の解説、作法の説明は書かない。決定と比較は ADR、作法の説明はガイドが持つ (ADR-0002)
- ガイドを `@` で import せず、「読む」とも書かない。import は `paths` があっても起動時に常時読み込まれ、案内は読まれない。規範は rules 自身が持つ (ADR-0005)
