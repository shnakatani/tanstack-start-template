...
- story から部品へ渡す `className` は layout に限る。消費側と同じ範囲で、`vite.config.ts` の `no-restyle` が `allow: ["layout"]` を持つ。外見を上書きする class は部品の variant にする。story は層の規則の `excludeFiles` に入り lint が鳴らないので、ここはレビューで見る (ADR-0021 / ADR-0022)
