...
- `argTypes` の `options` に `cva` の variant を写すときは、写す前に定義を読んで一致を確かめる。Storybook の型は `options?: readonly any[]` で中身を検査しないため、`cva` に足しても story は古いまま lint も型検査も鳴らない (2026-09-20 実測)
- story から部品へ `className` を渡さない。渡したいときは decorator で外側の要素に当てる。`ui/` `action/` `parts/` の story は `no-restyle` / `require-static-classes` の適用外 (`vite.config.ts` の `excludeFiles`) なので、渡しても lint は鳴らない (ADR-0020 / ADR-0021)
