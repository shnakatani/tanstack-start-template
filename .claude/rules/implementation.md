...
| 検索条件の変更                    | URL の `navigate` (Router の Transition)。打鍵中の一覧は debounce した値を `useDeferredValue` に通して `useSuspenseQuery` の key にし、fallback に落とさない (実例: `src/routes/notes/index.tsx`、ADR-0033)                                              |
