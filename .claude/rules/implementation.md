...
| mutation 成功後のダイアログ close | 閉じる時点は ADR-0016 の完了点の軸で選び、理由を実装近傍に書く (無いと形だけ写される)。(a) Action 内で `close()` + `void runAction()` / (b) `onSuccess` 先頭で `close()` し再取得の Promise を返す / (c) `closeAfterInvalidate` で再取得を await した後に `close()` |
