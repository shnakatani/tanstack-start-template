...
| スクリプトの単体テスト | スクリプト自身の変更         | `scripts/` (`scripts/checks/` を除く) | `vp test run --project scripts-tools`      |
...
- スクリプトの純粋関数は消費者で置き場所を決める。検査の判定か、実行口の外 (config / 別ディレクトリ) から使うものは `scripts/lib/`。1 つの実行口だけが使うものはその実行口の隣 `scripts/<ツール>/`
- 実行口と同じ拡張子の純粋関数は `scripts/<ツール>/lib/` へ下げる。直接実行するファイルと読み込まれるだけのファイルが見分けられなくなる (`scripts/contrast/report.ts` と `scripts/contrast/lib/`。`scripts/dev-env/` は実行口が `.sh` なので混ざらず平置き)
