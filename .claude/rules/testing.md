...
- 待つ口は 3 つ。locator の状態は `expect.element`、値を作って比べるなら `expect.poll`、matcher で表せない条件 (mock の呼び出し回数など) は `vi.waitFor` (ADR-0013)。コールバックを retry する `expect.poll` と `vi.waitFor` は中の同期読みがルールの対象外。`expect.element` は引数を 1 度しか解決しないので対象になる
...
- **否定形の `toHaveStyle` は使わない。** 宣言をブラウザが解釈できないと空になり、`.not` が素通りする。単位の書き忘れや綴り誤りが永久に緑になる。否定は `getComputedStyle` の読みで書く (ADR-0029)
