...
背景を持つスクロール領域は、器と中身の両方へ背景を置く。器側だけだと、スクロールで中身が器より高くなったときに axe がテキストの背景を解決できない (dequelabs/axe-core#621)。中身側だけだと、バーのぶんの余白が地のまま残る。実例は `src/components/parts/code-block.tsx`。
