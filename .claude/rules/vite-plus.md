...
| `overrides`         | テストファイルの型ルール緩和はここに書く (ADR-0004)。`categories` は持てず、`plugins` はトップレベルと違い継承集合へ**追加**する。違反の抑制には使わない (行単位で書く。ADR-0006)。規則そのものの適用範囲を層に合わせるときだけ `excludeFiles` を使う (ADR-0020) |
