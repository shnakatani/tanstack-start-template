...
| `jsPlugins`         | ESLint プラグインを oxlint 経由で読み込む。足す前に oxlint ネイティブで代替できないか確認する (JS 実行で lint 時間が伸びる)。エントリは `{ name, specifier }` で書く。抑制 directive をその `name` 以外で書くと無言で効かない (ADR-0004)。足したら `EXPECTED_JS_PLUGINS` にも足す |
