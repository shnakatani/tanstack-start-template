 ...
 * 適用先 glob の決め方は ADR-0039 が全ルールぶん持つ。対象の限定は `vite.config.ts` の
 ...
 *
 * 複数のルールがこの追跡に依拠している。ADR-0039 の「壊し方 (間接)」が固定しているのも
 ...
 * 観測の基準値と操作の後の観測を比べる書き方で、ADR-0041 が認める「2 回の観測を比べる」に当たる。
...
export const preferLocatorMethods = defineRule({
  meta: {
    type: "problem",
    docs: {
      description: "locator の同期読みを assert へ流さず、expect.element を通す (ADR-0039)",
    },
    messages: {
      syncRead:
        "locator の同期読みを expect() へ渡さない。expect.element を通す。matcher の無い実測は expect.poll のコールバックの中で読む。retry が無く、DOM の確定前に評価されると実装が正しくてもテストが落ちる (ADR-0039)",
    },
  },
  create(context) {
    return {
...
export const noFindElement = defineRule({
  meta: {
    type: "problem",
    docs: {
      description: "locator.findElement() を呼ばない。mount は expect.element で待つ (ADR-0040)",
    },
    messages: {
      findElement:
        "`locator.findElement()` を呼ばない。mount を待つなら `expect.element(locator).toBeInTheDocument()` を使う。この config は `actionTimeout` を置いており、`findElement()` は待ち時間が上限なしになる。要素が現れないとテストが `Test timed out` で落ち、locator の名前が出力から消える (ADR-0040)",
    },
  },
  create(context) {
    return {
...
/**
 * 算出値を数値へ変える呼び出し。ADR-0041 が肯定形の書き方として勧めているので、
...
export const noNegatedStyleLiteral = defineRule({
  meta: {
    type: "problem",
    docs: {
      description: "スタイルをリテラルとの否定で確かめない (ADR-0041)",
    },
    messages: {
      negatedStyleLiteral:
        "スタイルをリテラルとの否定で確かめない。`not.toHaveStyle` は宣言を解釈できないと素通りし、算出値との `not.toBe` は単位や綴りが 1 つ外れると潰れた状態でも通る。1 回の観測から数値を出すか、期待する値そのものと肯定で比べる (ADR-0041)",
    },
  },
  create(context) {
...
export const noBareAbsenceAssertion = defineRule({
  meta: {
    type: "problem",
    docs: {
      description: "不在の assert は expectAbsent / expectRemoved を通す (ADR-0041)",
    },
    messages: {
      bareAbsence:
        "`expect.element(...).not.toBeInTheDocument()` を直に書かない。最初から出ないなら `expectAbsent(locator)`、在る状態から消えるのを待つなら `expectRemoved(locator)` を使う (`src/test/absent.ts`)。同じ matcher なので、名前を付けないとどちらのつもりかが字面で読めない。`expectAbsent` は要素が無ければ 1 回目で通るので、同じ操作の効果を表す肯定 assert を先に置く。無いと何も検証していない (ADR-0041)",
    },
  },
  create(context) {
    return {
