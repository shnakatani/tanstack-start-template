...
tester.run("prefer-locator-methods", preferLocatorMethods, {
  valid: [
    ...
    // 束縛して matcher の期待値に使う形は、観測の基準値との比較 (ADR-0041)
    ...
    // `vi.waitFor` も retry の口 (ADR-0036)。assert へ直に届く形をここで固定する
  ...
  invalid: [
    ...
    {
      // 要素そのものの束縛は、matcher の期待値に来ても基準値ではない (ADR-0039)
    ...
    // 変数へ束縛してから渡す形。スコープ解析が外れるとここだけ無言で通る (ADR-0039)
    {
...
tester.run("no-find-element", noFindElement, {
  ...
  invalid: [
    ...
    {
      // timeout を明示しても呼ばない。予算を呼び出しごとに持つ形は 2026-09-22 に撤去した (ADR-0040)
...
tester.run("no-negated-style-literal", noNegatedStyleLiteral, {
  valid: [
    ...
    // ADR-0041 が推奨する肯定形。src/components/ui/dialog.test.tsx の綴り
  ...
  invalid: [
    ...
    {
      // ADR-0041 が推奨する肯定形を否定へ倒した退行。src の 8 箇所がこの綴り
