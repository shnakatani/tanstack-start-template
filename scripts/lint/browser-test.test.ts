...
tester.run("prefer-locator-methods", preferLocatorMethods, {
  valid: [
    ...
    // 束縛して matcher の期待値に使う形は、観測の基準値との比較 (ADR-0049)
    ...
    // `vi.waitFor` も retry の口 (ADR-0044)。assert へ直に届く形をここで固定する
  ...
  invalid: [
    ...
    // 変数へ束縛してから渡す形。スコープ解析が外れるとここだけ無言で通る (ADR-0047)
    {
...
tester.run("no-negated-style-literal", noNegatedStyleLiteral, {
  valid: [
    ...
    // ADR-0049 が推奨する肯定形。src/components/ui/dialog.test.tsx の綴り
  ...
  invalid: [
    ...
    {
      // ADR-0049 が推奨する肯定形を否定へ倒した退行。src の 8 箇所がこの綴り
