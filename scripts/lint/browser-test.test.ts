...
tester.run("prefer-locator-methods", preferLocatorMethods, {
  valid: [
    ...
    // 束縛して matcher の期待値に使う形は、観測の基準値との比較 (ADR-0037)
    ...
    // `vi.waitFor` も retry の口 (ADR-0033)。assert へ直に届く形をここで固定する
  ...
  invalid: [
    ...
    {
      // 要素そのものの束縛は、matcher の期待値に来ても基準値ではない (ADR-0036)
    ...
    // 変数へ束縛してから渡す形。スコープ解析が外れるとここだけ無言で通る (ADR-0036)
    {
...
tester.run("no-negated-style-literal", noNegatedStyleLiteral, {
  valid: [
    ...
    // ADR-0037 が推奨する肯定形。src/components/ui/dialog.test.tsx の綴り
  ...
  invalid: [
    ...
    {
      // ADR-0037 が推奨する肯定形を否定へ倒した退行。src の 8 箇所がこの綴り
