...
/**
 * design system を著作する層 (ADR-0015)。`@shadcn/lint` の 2 軸をここから導出する。
...
export default defineConfig({
  ...
  lint: {
    ...
    // (ADR-0031)。name は診断コード・rules のキー・抑制 directive で共有される名前になる
    jsPlugins: [
      { name: "shadcn", specifier: "@shadcn/lint" },
      // story は `storybook/test` 経由で testing-library の API をそのまま使う。oxlint は
      // testing-library をネイティブに持たないため ESLint plugin として載せる (ADR-0012)
      { name: "testing-library", specifier: "eslint-plugin-testing-library" },
      // ブラウザテストの assert に locator を渡させる自前ルール。上流の
      // @vitest/eslint-plugin は browser mode の locator を対象にしたルールを持たない (ADR-0043)
    ...
    settings: {
      shadcn: {
        ...
        // 推奨しており、テンプレート利用者がそのまま書けるようにする (ADR-0030)。
    ...
    // recommended を基準に rules へ名指しする (ADR-0011)。
    ...
    rules: {
      ...
      // -- eslint コア: @eslint/js の recommended (ADR-0011) --
      ...
      // -- eslint コア: typescript-eslint の eslint-recommended が error にする分 (ADR-0011) --
      ...
      // correctness に入っていない分 (ADR-0011)。オプションも基準に揃える --
      ...
      // -- react: eslint-plugin-react の recommended と jsx-runtime (ADR-0011) --
      ...
      // -- react-hooks: eslint-plugin-react-hooks の recommended-latest (ADR-0011)。
      ...
      // 欠陥として扱う (ADR-0018)。oxlint では restriction のため既定 off
      ...
      // -- import: eslint-plugin-import の recommended (ADR-0011) --
      "import/export": "error",
      ...
      // -- promise: eslint-plugin-promise の recommended (ADR-0011) --
      ...
      // 持たないコードベースでは、状態機械の遷移値を next と名付けただけで鳴る (ADR-0011)
      ...
      // -- jsdoc: eslint-plugin-jsdoc の recommended-typescript (ADR-0011) --
      ...
      // -- oxc: 上流に対応する設定がない。correctness と perf で拾う (ADR-0011) --
      // 公式が示す修正は元オブジェクトの破壊的更新で、確保を 1 回に減らすもの。読み出し
      // 結果や fixture を壊せない箇所では新規オブジェクトを作るしかなく、確保回数が
      // spread と同じになって効果が消える (ADR-0011)
      "oxc/no-map-spread": "off",

      // -- vitest: @vitest/eslint-plugin の recommended (ADR-0011) --
      ...
      // -- jsx-a11y: eslint-plugin-jsx-a11y の recommended (ADR-0011) --
      ...
      // -- shadcn: ルールは設計判断と対にして 1 つずつ名指しする (ADR-0031)。ここに置くのは
    ...
    overrides: [
      {
        ...
        // である。件数は下の rules と ADR-0012 の表が持つ
        // 拡張子は companion-files.ts が唯一の定義。`.stories.ts` を置いても外れない
        files: storyGlobs("**/"),
        rules: {
          // eslint-plugin-testing-library の flat/react (ADR-0012)
          ...
          // (掴む理由を実装近傍に書く運用を lint 抑制へ置き換えることになる。ADR-0012)
      ...
      },
      {
        ...
        // いるか」を見る規則で、design system 自身の内部には意味を持たない (ADR-0015)。緩和では
      ...
      {
        // ブラウザテストと、そこへ locator を配る helper が対象 (ADR-0043)。テスト本文だけに
        ...
        rules: {
          ...
          // `locator.findElement()` を止める (ADR-0044)。正当な呼び出し元は無く、除外も置かない
          "browser-test/no-find-element": "error",
          // スタイルの否定 assert が素通りする形を止める (ADR-0045)
          "browser-test/no-negated-style-literal": "error",
          // 不在の assert を helper の名前で読み分けさせる (ADR-0045)
      ...
      },
      {
        ...
        // (ADR-0013)。story 自身も止める。story を import すると
        ...
        // (ADR-0048)、`preview.tsx` が `src/test/viewport-sizes.ts` を読む
        files: ["src/**", "scripts/**"],
              ...
              patterns: [
                {
                  ...
                  message:
                    "テストと story だけが使うコード。アプリのコードから import しない (ADR-0013)",
                },
              ],
            },
          ],
    ...
    tanstackStart({
      ...
      importProtection: {
        client: {
          ...
          // *.server.* 側が遮断する (ADR-0014)。この配列は既定を merge せず置換するので
