...
/**
 * design system を著作する層 (ADR-0016)。`@shadcn/lint` の 2 軸をここから導出する。
...
export default defineConfig({
  ...
  lint: {
    ...
    jsPlugins: [
      ...
      // testing-library をネイティブに持たないため ESLint plugin として載せる (ADR-0013)
    ...
    // recommended を基準に rules へ名指しする (ADR-0012)。
    ...
    rules: {
      ...
      // -- eslint コア: @eslint/js の recommended (ADR-0012) --
      ...
      // -- eslint コア: typescript-eslint の eslint-recommended が error にする分 (ADR-0012) --
      ...
      // correctness に入っていない分 (ADR-0012)。オプションも基準に揃える --
      ...
      // -- react: eslint-plugin-react の recommended と jsx-runtime (ADR-0012) --
      ...
      // -- react-hooks: eslint-plugin-react-hooks の recommended-latest (ADR-0012)。
      ...
      // 欠陥として扱う (ADR-0019)。oxlint では restriction のため既定 off
      ...
      // -- import: eslint-plugin-import の recommended (ADR-0012) --
      "import/export": "error",
      ...
      // -- promise: eslint-plugin-promise の recommended (ADR-0012) --
      ...
      // 持たないコードベースでは、状態機械の遷移値を next と名付けただけで鳴る (ADR-0012)
      ...
      // -- jsdoc: eslint-plugin-jsdoc の recommended-typescript (ADR-0012) --
      ...
      // -- oxc: 上流に対応する設定がない。correctness と perf で拾う (ADR-0012) --
      // 公式が示す修正は元オブジェクトの破壊的更新で、確保を 1 回に減らすもの。読み出し
      // 結果や fixture を壊せない箇所では新規オブジェクトを作るしかなく、確保回数が
      // spread と同じになって効果が消える (ADR-0012)
      "oxc/no-map-spread": "off",

      // -- vitest: @vitest/eslint-plugin の recommended (ADR-0012) --
      ...
      // -- jsx-a11y: eslint-plugin-jsx-a11y の recommended (ADR-0012) --
    ...
    overrides: [
      {
        ...
        // である。件数は下の rules と ADR-0013 の表が持つ
        // 拡張子は companion-files.ts が唯一の定義。`.stories.ts` を置いても外れない
        files: storyGlobs("**/"),
        rules: {
          // eslint-plugin-testing-library の flat/react (ADR-0013)
          ...
          // (掴む理由を実装近傍に書く運用を lint 抑制へ置き換えることになる。ADR-0013)
      ...
      },
      {
        ...
        // いるか」を見る規則で、design system 自身の内部には意味を持たない (ADR-0016)。緩和では
      ...
      },
      {
        ...
        // (ADR-0014)。story 自身も止める。story を import すると
        ...
        // (ADR-0049)、`preview.tsx` が `src/test/viewport-sizes.ts` を読む
        files: ["src/**", "scripts/**"],
              ...
              patterns: [
                {
                  ...
                  message:
                    "テストと story だけが使うコード。アプリのコードから import しない (ADR-0014)",
                },
              ],
            },
          ],
    ...
    ignorePatterns: [
      ...
      ".agents/**",
      ".claude/skills/**",
      // registry の生成時 baseline (ADR-0027)。上流のコードをそのまま保存する記録なので
    ...
    tanstackStart({
      ...
      importProtection: {
        client: {
          ...
          // *.server.* 側が遮断する (ADR-0015)。この配列は既定を merge せず置換するので
