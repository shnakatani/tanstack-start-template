...
既定集合を明示して積む決定は ADR-0008、ルールの選定基準は ADR-0009、色の統制に `@shadcn/lint` を使う決定は ADR-0029、自前のルールを JS plugin で書く決定は ADR-0041 が持つ。
...
1. 上流 recommended に入っているかを確かめる (基準表は ADR-0009)
2. 入っていないものを足すなら、ADR-0009「基準から外れる名指し」の表に理由と一緒に追記する
...
oxlint (Vite+ 同梱) の minor 以上の更新が Dependabot の PR で来たら、ADR-0009 の基準表のプラグインごとに上流の一覧と突き合わせる。
typescript-eslint は依存に入っていないので、`strict` の改訂を知らせるものが無い。ADR-0009 を読み直すときに追随する。
...
- registry コードの中の抑制は、ADR-0024 の許容リストにも記録する
...
`scripts/lint/` に置き、`vite.config.ts` の `lint.jsPlugins` から読む (ADR-0041)。実例は `scripts/lint/browser-test.ts` とそのテスト `scripts/lint/browser-test.test.ts`。
...
| 落とし穴                                                                                          | 起きること                                                                                                  | 避け方                                                                                                         |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| トップレベルの `plugins` は既定集合を置換する                                                     | spread を落とすと `typescript` を含む既定の検査が消える                                                     | `OXLINT_DEFAULT_PLUGINS` を spread して積む (ADR-0008)                                                         |
...
| `overrides` の `files` の否定 glob (`!**/*.test.ts`) は除外として効かない                         | oxlint 1.79.0 で、最小構成で除外されなかった (2026-09-14)                                                   | `excludeFiles` を使う (ADR-0011)                                                                               |
| `rules` に `"warn"` と書く                                                                        | `vp check` は warn を exit code に出さない。新しいコードの違反が通る                                        | `"error"` で書く。移行を待つ間も warn に下げない (ADR-0010 の `no-debugging-utils`、ADR-0041 の移行と同じ判断) |
...
| `settings.shadcn.componentImports` や `variantFunctions` を消しても、`--print-config` に `settings.shadcn` が出ない | 自作部品が規則から見えなくなり、routes からの上書きが素通りする。宣言が消えたことを見張るものは無い | 消すときは probe で自作部品への上書きが報告されるかを確かめる                                                                                            |
| 引数を取らない関数を `mergeFunctions` へ登録する                                                                    | 規則を通しながら、戻り値の中身の検査を落とせる (2026-09-19 実測)                                    | 抜け道として使わない。variant 関数は `variantFunctions` へ宣言する (ADR-0028)                                                                            |

## explanation

### `correctness` と基準のずれ

oxlint のカテゴリは実装者がルールを分類した軸で、上流の recommended とは一致しない (ADR-0009)。そのため次のずれが起きる。

- 基準が off にするルールでも、`correctness` に入っていればカテゴリ側が勝つ。`rules` で明示的に off にしないと有効のまま残る
- `vitest` プラグインはテストファイル以外にも効く。行頭がテストの呼び出しに見えるコメントは `no-commented-out-tests` で報告される
...
| `invariant` / `rule-suppression` / `syntax` / `todo`                                   | `restriction`     | off。`todo` は Compiler の未実装による bail out で、欠陥として扱わない (ADR-0016) |
| `capitalized-calls` / `exhaustive-effect-dependencies` / `hooks` / `memo-dependencies` | `suspicious`      | off。上流が既定から外している                                                     |

どれを名指しして引き上げるかは ADR-0009「React Compiler のルールは eslint-plugin-react-hooks を基準にする」が決める。
