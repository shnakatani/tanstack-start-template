...
既定集合を明示して積む決定は ADR-0010、ルールの選定基準は ADR-0011、色の統制に `@shadcn/lint` を使う決定は ADR-0031、自前のルールを JS plugin で書く決定は ADR-0043 が持つ。
...
- registry コードの中の抑制は、ADR-0026 の許容リストにも記録する
...
| `rules` に `"warn"` と書く                                                                        | `vp check` は warn を exit code に出さない。新しいコードの違反が通る                                        | `"error"` で書く。移行を待つ間も warn に下げない (ADR-0012 の `no-debugging-utils`、ADR-0043 の移行と同じ判断) |
