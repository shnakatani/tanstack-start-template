...
| [lint.md](lint.md)                                             | lint の運用 (解決後の設定での確かめ方、プラグインの足し方、testing-library の範囲、off の条件とテストの緩和、抑制、`require-static-classes` と variant 関数、自前のルール、設定と JS plugin の落とし穴) |
...
| [testing.md](testing.md)                                       | テストの書き方 (待つ口、同期読みの書き換え、クリックの発火、animation を戻すテスト、否定を肯定で書く、assert の予算、route の wrapper、viewport、検査スクリプトの置き方)                                |
