...
- input の上に疑似要素や別の要素を重ねて、hit 領域を広げない。重なった要素が pointer を受け、本体がクリックを受け取れなくなる。テストでは Playwright の hit-target 検査で click が落ちる (ADR-0042)。registry の `Input` 単体は `src/components/ui/input-pointer.test.tsx` が見る
