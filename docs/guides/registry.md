...
| 決定                                                                             | ADR      |
| -------------------------------------------------------------------------------- | -------- |
| registry との乖離は生成時 baseline との 3-way で判別し、台帳の行に限る           | ADR-0024 |
...
5. story を書く。消費側からの import が 0 件でも書く (ADR-0048)
...
# 再生成するときも書き込むが、vendor した部品の契約なのでそのまま受け入れる (ADR-0024)
...
統制するのは「してはいけない改変」ではなく、再生成と上流の追随を安全に回すために「何を変えたか」を残すことである。そのための記録が baseline と台帳で、変える前にまず公式の推奨に合わせ、実機で見てから判断する (ADR-0024「追加と削除の基準」)。
