...
| 順  | 当てはまる場面                                                                                  | 直し方                                                                                                   | 出典                                                                                                                                                                                         |
| --- | ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | 公式の例が既定のまま使っている場面か、公式のノブ (data 属性・CSS 変数・prop) で同じ見た目になる | 上書きをやめて既定かノブに寄せる                                                                         | shadcn skill `customization.md`「Customizing Components」の順 (既存の variant → className → 新しい variant → 包み): https://github.com/shadcn-ui/ui/blob/main/skills/shadcn/customization.md |
...
| 3   | 画面ごとに値が変わるのが正しい変更                                                              | `no-restyle` の `contracts` で、その部品のその範囲だけを許す。粒度は class グループか個別の class にする | shadcn-ui/lint `docs/design-systems.md`: https://github.com/shadcn-ui/lint/blob/main/docs/design-systems.md                                                                                  |
...
`mise run contrast` の比は、同じ色を渡せば axe の `getContrast` と一致する (「測り方の限界」)。突き合わせるときは、`measurePair` の結果を `toHex` で渡して次と比べる。継続して検査はしない。
...
ADR-0024 の Context は、上流生成物の値を oklch から sRGB へ変換し、alpha を持つ値は下地へ合成してから比を取ったと書く。その変換器 (`scripts/contrast/`) をリポジトリに置くのは、測り方が無いと文書の数値を誰も追試できず、トークンを動かしたあとの再測もできないためである。

書き写した比はトークンに追随しない。2026-09-21 にトークンを刷新したとき、`src/components/parts/segmented-radio-group.tsx` と `src/components/parts/data-table.tsx` に書き写していた 3 箇所が古いまま残り、別の文書と `data-table.tsx` が `opacity-50` の比を別の値で書いていた (同じ主張を 2 箇所へ写したため)。
