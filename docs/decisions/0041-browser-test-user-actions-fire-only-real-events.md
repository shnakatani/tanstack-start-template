# ADR-0041: ブラウザテストのユーザー操作は実イベントだけで発火する

- Status: Accepted
- Date: 2026-09-22
- 関連: ADR-0040 (待機は retry API に委ねる。本 ADR は発火の側)、ADR-0021 (Action 層。「二重発火は state だけで塞ぐ」判断は、本 ADR の検証方法を前提にする)
...
**バックドロップは再現しない。** Dialog / AlertDialog の中のボタンを押す 4 箇所 (`src/routes/notes/index.test.tsx` の `confirmDelete` とキャンセル、`src/routes/notes/-components/note-create-dialog.test.tsx` の `clickSave` とキャンセル) は、`locator.click()` で browser project を全件走らせると全件通る。ADR-0042 の animation 無効化が効いているためではない。registry の AlertDialog を開いて実行ボタンを押す最小構成で、`enableAnimations()` の有無にかかわらず `.click()` が 130ms 台で通り、ハンドラが 1 回呼ばれる。
...
| 合成イベントの同期 2 連射を残し、実装に ref のフラグを持つ        | 起きない事象への防御をテストが要求する形。react.dev の `disabled={pending}` の形から外れる (ADR-0021)                                              | 却下     |
