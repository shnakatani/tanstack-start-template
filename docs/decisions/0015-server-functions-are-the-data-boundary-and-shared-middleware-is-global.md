...
- 認可の base builder は、ロールによる出し分けが必要になった時点で足す。`createServerFn` の直接 import を lint で禁じるかも、その時点で ADR-0009 の選定基準に当てて判断する。禁じられること自体は oxlint の `no-restricted-imports` で確認した (下記「出典」)
