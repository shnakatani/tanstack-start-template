# ADR-0036: axe の緑を測った証明とせず、測った件数を別に要求する

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0035 (`incomplete` を落とす層) / ADR-0030 (1.4.11 を axe が持たない) / ADR-0031 (`::placeholder` を axe が誤って評価する)
...
| 案                                                       | 評価                                                                                       | 採否     |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------ | -------- |
| `violations` が 0 件なら合格とする (`addon-a11y` の既定) | 検査が空振りしても緑になる。`color-contrast` は画面に出ていない要素を合格にする            | 却下     |
| 測った件数 (`passes`) が 0 でないことを別に要求する      | 空振りの緑を落とせる。ルール単位では見ない粗い形で、`expectNoA11yViolations` に 1 行で入る | **採用** |
