...
- Status: Accepted
- Date: 2026-09-19
- 関連: ADR-0027 (`@shadcn/lint` のルールの選定)、ADR-0024 (registry コードの改変は許容リストで統制)、ADR-0012 (features / routes の配置の原則)、ADR-0019 (Action 層)、ADR-0021 (DataTable 部品の配置)
...
| 上流 README の形 (トップレベル `rules` + 著作側の override で `"off"`) | 規則を off にする形は、適用範囲の宣言と違反の緩和を設定の字面で区別できない。ADR-0024 が分ける違反の抑制 (行単位) と適用範囲 (`excludeFiles`) の区別に外れる                                           | 却下     |
