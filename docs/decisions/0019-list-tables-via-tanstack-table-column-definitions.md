...
**一覧テーブルは TanStack Table v9 の列定義 (`createColumnHelper`) を SSOT にし、shadcn「Data Table」と同じ構成の `DataTable` 部品 (`src/components/parts/data-table.tsx`) が registry の `Table` に `FlexRender` で描く。楽観表示の設計は ADR-0016 のままで、data 配列を組み立てる側に閉じる。**
...
| shadcn の `DataTable` 部品を持つ               | 描画の骨組み (`FlexRender`、`scope="col"`、devtools、空行) が 1 箇所に集まり、ページは列定義と data を渡すだけになる。置き場は registry ではない自作部品の `src/components/parts/` (ADR-0006 で `components/ui/` は registry 専用、ADR-0020 で `parts/` に分離) | **採用** |
...
- `.claude/rules/directory-structure.md` が列定義の置き場を持つ。route-local の `-lib/` / `-hooks/` と、features と route のどちらに置くかの基準は ADR-0012 (Revised) が持つ。`.claude/rules/implementation.md` が見出しセルの `scope="col"` を持つ
