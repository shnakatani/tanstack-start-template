...
| cell への関数    | module 定数は import で足りる。削除確認の handle は trigger (列定義) と Root を描くページの両方が import する `-components/note-delete-dialog.ts` に置く。Root の実体が共有部品 (`DeleteConfirmDialog`) なら wrapper を挟まない (Base UI が求めるのは Trigger と Root が同じ handle を持つことだけ)。cell が画面の callback を要するようになったら `tableMeta` (`metaHelper`) で渡す |
