...
| 列定義の置き場   | その画面だけの列は `routes/<path>/-components/<画面>-columns.tsx` (shadcn の `columns.tsx` と同じ分離。cell が JSX を持つので部品側)。cell が描く部品 (削除トリガー等) は同じ `-components/` に切り出す。行の組み立てや handle など React に依存しないものは `routes/<path>/-lib/` (テスト同梱)。ドメインを跨いで使う列は `src/features/<domain>/`                                                     |
...
| 行の型           | 確定行と保存中の行の union (`routes/<path>/-lib/note-rows.ts` の `NoteRow`: `{ kind: "saved"; note; isDeleting } \| { kind: "creating" } & CreatingRow`)。cell が `kind` で分岐するので、楽観行も同じ列定義で描ける                                                                                                                                                                                    |
...
- `.claude/rules/directory-structure.md` が列定義の置き場と、route-local の React 非依存ロジックを置く `-lib/` と hook を置く `-hooks/` を持つ。`.claude/rules/implementation.md` が見出しセルの `scope="col"` を持つ
