...
| cell への関数    | module 定数は import で足りる。削除確認の handle は `-lib/note-delete-dialog-handle.ts` が持ち、trigger を描く cell 部品 (`-components/note-actions-cell.tsx`) と Root を描くページの両方がそこに依存する (列定義 → cell 部品 → handle の向きで、逆ではない)。Root の実体が共有部品 (`DeleteConfirmDialog`) なら wrapper を挟まない。cell が画面の callback を要するようになったら `tableMeta` (`metaHelper`) で渡す       |
...
- `.claude/rules/directory-structure.md` が列定義の置き場と、route-local の非コンポーネントを置く `-lib/` と hook を置く `-hooks/` を持つ。`.claude/rules/implementation.md` が見出しセルの `scope="col"` を持つ
