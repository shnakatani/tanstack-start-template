...
| 列定義の置き場   | その画面だけの列は `routes/<path>/-components/<画面>-columns.tsx` (shadcn の `columns.tsx` と同じ分離、ADR-0012 の route-local)。ドメインを跨いで使う列は `src/features/<domain>/`                                                                                                                                                            |
...
| 行の型           | 確定行と保存中の行の union (`{ kind: "saved"; note; isDeleting } \| { kind: "creating"; submittedAt; input }`)。cell が `kind` で分岐するので、楽観行も同じ列定義で描ける                                                                                                                                                                     |
| data の組み立て  | ページが `[...creatingRows, ...notes]` に `isDeleting` を派生させて `DataTable` に渡す。`useMemo` は書かない (ADR-0009、React Compiler)。`getRowId` で `creating-<submittedAt>` / `saved-<id>` を付け、React の key にも使う                                                                                                                  |
...
| cell への関数    | module 定数 (削除確認の handle) は import で足りる。cell が画面の callback を要するようになったら `tableMeta` (`metaHelper`) で渡す                                                                                                                                                                                                           |
...
- `.claude/rules/directory-structure.md` が列定義の置き場を、`.claude/rules/implementation.md` が見出しセルの `scope="col"` を持つ
