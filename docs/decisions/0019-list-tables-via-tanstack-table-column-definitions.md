...
| 列定義の置き場   | その画面だけの列は `routes/<path>/-lib/<画面>-columns.ts` (JSX を持たない設定)。描画を持つ列は `cell` に `-components/` の部品の参照を渡す (`FlexRender` が cell の context を props にして描く。TanStack Table「Flex Render」)。行の組み立てや handle も `-lib/` (テスト同梱)。ドメインを跨いで使う列は `src/features/<domain>/`                                                                      |
