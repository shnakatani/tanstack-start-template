# ADR-0043: a11y の検査は project ではなく tag で分ける

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0040 (`incomplete` を落とす層) / ADR-0041 (axe で何を測り何を測らないか) / ADR-0057 (story を検査の単位にする) / ADR-0046 (incomplete に噛まれた事故)
...
| 専用のテストファイルへ分ける     | 却下 | テストの置き場所は壊れる原因で分けており、a11y は挙動テストと同じ原因で壊れる                                         |
| 相乗りの assert も降ろして分ける | 却下 | 「挙動テストの途中の状態を測る assert には tag を付けない」のとおり。分離の見返りはラベルだけで、操作の再現が重複する |
...
- story 側の a11y は `addon-a11y` が全 story へ一律に当てるので、tag の対象外 (ADR-0042)
