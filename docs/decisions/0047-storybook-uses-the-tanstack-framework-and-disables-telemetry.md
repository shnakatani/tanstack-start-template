# ADR-0047: Storybook は TanStack 専用の framework で導入し、telemetry を切る

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0048 (story を状態のカタログにする) / ADR-0007 (ツールチェーン。明示で潰す既定の揃え方)
...
telemetry は `core.disableTelemetry` で切る。既定で有効で、実行したコマンド・バージョン・addon 一覧・story とコンポーネントの件数を送る。このテンプレートから作られる全プロジェクトへ配られる設定なので、`envDir: false` や `disable_tools` (ADR-0007) と同じく明示で潰す側に揃える。
