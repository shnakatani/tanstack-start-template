# ADR-0029: 色は `@theme` と `@shadcn/lint` の 2 層で semantic token に閉じ込める

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0009 (lint ルールの選定基準)、ADR-0024 (行単位の抑制の許容リスト)、ADR-0013 (`no-restyle` の適用範囲)、ADR-0028 (`require-static-classes` と variant 関数の宣言)、ADR-0010 (`eslint` を必須 peer に持つもう 1 つの経路)
