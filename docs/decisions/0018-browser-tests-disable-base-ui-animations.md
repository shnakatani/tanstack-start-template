...
| `prefers-reduced-motion: reduce` のエミュレーション (`src/styles.css` のブロックが CSS の animation / transition を 0.01ms にする) | 2026-09-14 時点では却下した (context 単位で、テスト単位に戻すには CDP が要る)。2026-09-22 に CDP `Emulation.setEmulatedMedia` で `beforeEach` から per-test に立てる形で採用。次のテストの `beforeEach` が立て直すので、戻す経路を持たない (`parkMouse` と同じ形)。rect 実測が `expect.poll` へ移り、enter animation を走らせる理由が無くなった | **採用 (2026-09-22)** |
