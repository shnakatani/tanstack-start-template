# ADR-0024: 一覧の絞り込み条件は URL の search param が持ち、loaderDeps で loader に渡す

- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0017 (ドメイン型は valibot スキーマから導出する)、ADR-0019 (ナビゲーションは Router の Transition に任せる)、ADR-0025 (入力欄と一覧の描画)、ADR-0046 (wrapper のテスト)
...
- `src/routes/notes/index.tsx` は `Route` と、export しない wrapper だけを持つ (ADR-0014)。入力欄と一覧の描画は ADR-0025 が持つ。テストは ADR-0046、サーバ側の LIKE は `src/server/db/like-pattern.ts`
