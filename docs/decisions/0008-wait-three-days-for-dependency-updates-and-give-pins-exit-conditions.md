# ADR-0008: 依存更新は待機 3 日で統一し、pin には出口条件を書く

- Status: Accepted
- Date: 2026-09-02
- 関連: ADR-0007 (Vite+ が版を管理する制約)、ADR-0018 (React Compiler が要求する依存)
...
`.github/zizmor.yml` の `rules.dependabot-cooldown.config.days: 3` は、`cooldown` がこの待機を下回ったら CI を落とす下限である (ADR-0009)。zizmor 1.30.1 の既定は 7 日 (2026-09-23 確認) だが、7 日案は下の「検討した選択肢」で却下しているので、下限を待機に合わせる。待機を変えるときはこの値も揃える。
