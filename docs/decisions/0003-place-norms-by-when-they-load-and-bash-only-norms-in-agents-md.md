...
- ファイルを読む前に要る規範 (新しいファイルを書き始める前の手順) と、Bash で打つだけの操作に要る規範は、`AGENTS.md` 自身に書く。`AGENTS.md` から rules やガイドを「読む」と言葉で誘導しても、読まれる保証は無い (ADR-0005 の測定で 2 回とも読まれなかった)。新しいファイルを書くだけでは `paths` の rules は読み込まれない
