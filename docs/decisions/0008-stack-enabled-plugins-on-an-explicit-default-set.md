# ADR-0008: 有効にするプラグインは既定集合を明示して積む

- Status: Accepted
- Date: 2026-08-17
- 関連: ADR-0009 (ルールの選定方法)
...
| registry コードを lint 対象から除外する  | ADR-0026 の既定は改変して許容リストへ記録する運用で、除外はそれを反転させる | 却下     |

## Consequences

- 既定検査が、追加プラグインの設定によって無言で消えなくなる
- プラグインを追加すると既存違反が一度に顕在化する。件数が多く別の判断軸を持つ検査は、独立した変更へ分ける
- 行単位の抑制は領域を問わず使ってよい。registry コードで追加で要るのは ADR-0026 の許容リストへの記録であって、抑制の可否そのものではない
