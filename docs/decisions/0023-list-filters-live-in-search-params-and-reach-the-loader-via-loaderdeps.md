...
- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0016 (ドメイン型は valibot スキーマから導出する)、ADR-0018 (ナビゲーションは Router の Transition に任せる)、ADR-0024 (入力欄と一覧の描画)、ADR-0051 (wrapper のテスト)
...
- `src/routes/notes/index.tsx` は `Route` と、export しない wrapper だけを持つ (ADR-0013)。入力欄と一覧の描画は ADR-0024 が持つ。テストは ADR-0051、サーバ側の LIKE は `src/server/db/like-pattern.ts`
