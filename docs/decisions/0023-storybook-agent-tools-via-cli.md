...
- `write-story` skill は上流の規約で、このリポジトリの決定と食い違う箇所がある (「ALWAYS write a Storybook story for any component written」「Simulate key user flows」)。上限は ADR-0022 が持つ。ADR に上限が書かれていない項目は上流の既定値が入る
