# ADR-0023: Storybook の agent 向けツールは同梱の CLI を使い、MCP addon は入れない

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0002 (ツールチェーン) / ADR-0022 (Storybook を状態のカタログとして導入)
...
- `storybook --help` に `skills` と `tools` が出ないため、`AGENTS.md` から外れた瞬間に見落とされる。サイズ制限 (`.claude/rules/docs.md`「AGENTS.md」) の中で維持する必要がある
- `write-story` skill は上流の規約で、このリポジトリの決定と食い違う箇所がある (「ALWAYS write a Storybook story for any component written」「Simulate key user flows」)。上限は ADR-0022 / ADR-0055 が持つ。ADR に上限が書かれていない項目は上流の既定値が入る
