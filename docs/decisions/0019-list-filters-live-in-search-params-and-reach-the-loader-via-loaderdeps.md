...
- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0013 (ドメイン型は valibot スキーマから導出する)、ADR-0015 (ナビゲーションは Router の Transition に任せる)
...
- `src/routes/notes/index.tsx` は `Route` と、export しない wrapper だけを持つ (ADR-0010)。入力欄と一覧の描画は `docs/guides/lists-and-search.md`「検索の入力欄を組む」にある。テストは `docs/guides/testing/route-wrappers.md`「route の wrapper をテストする」、サーバ側の LIKE は `src/server/db/like-pattern.ts`
