# ADR-0016: server function をデータ境界とし、全 fn 共通の middleware は global に載せる

- Status: Accepted
- Date: 2026-09-06
- 関連: ADR-0011 (lint ルールの選定基準)
...
- 認可の base builder は、ロールによる出し分けが必要になった時点で足す。`createServerFn` の直接 import を lint で禁じるかも、その時点で ADR-0011 の選定基準に当てて判断する。禁じられること自体は oxlint の `no-restricted-imports` で確認した (下記「出典」)
