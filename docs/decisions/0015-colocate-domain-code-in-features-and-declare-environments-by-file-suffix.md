# ADR-0015: ドメインに属するコードは `src/features/<domain>/` へ集め、環境はファイル名の接尾辞で宣言する

- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0017 (server function のデータ境界)
...
| 現状を保ち rules の文言だけ直す                                 | 差分は最小だが、テンプレートから作るプロジェクトへ構造の指針を渡さない                                                                  | 却下     |
