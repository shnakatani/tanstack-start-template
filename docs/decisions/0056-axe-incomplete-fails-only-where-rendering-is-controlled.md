# ADR-0056: axe の incomplete は描画を統制できる層でだけ落とし、`color-contrast` の incomplete は外さない

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0026 (緑を測った証明としない) / ADR-0057 (層ごとの役割と抑制の置き場所) / ADR-0027 (a11y の検査を tag で分ける) / ADR-0018 (incomplete に噛まれた事故と回避策) / ADR-0017 (a11y 検査の対象) / ADR-0054 (story を検査の単位にする)
