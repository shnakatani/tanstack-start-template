# ADR-0020: イベントハンドラは同期関数とし、非同期処理は内側へ閉じる

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0011 (`typescript/no-misused-promises` を名指しする基準)、ADR-0019 (mutation を伴う操作の Action)
...
mutation を伴う操作は、その同期ハンドラの内側で `startTransition` に非同期関数を渡す形 (Action) にし、pending は Transition から取る。この判断は ADR-0019 が持つ。
