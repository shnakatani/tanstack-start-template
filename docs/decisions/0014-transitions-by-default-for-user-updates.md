...
- Status: Accepted
- Date: 2026-09-13
- Revised: 2026-09-14 (完了点とブロック範囲の選択を ADR-0016 の軸へ移し、再取得完了まで待つこととトリガーの全体無効化を既定から外した)
...
| 再取得と close          | `onSuccess` は完了点によらず再取得の Promise を返す (TanStack Query は `onSuccess` の Promise を待つので、その間 `isPending` が続く)。閉じる時点は ADR-0016 で選び、(c) では `closeAfterInvalidate` (`src/lib/close-after-invalidate.ts`) でその後に `handle.close()`、(b) では先頭で `close()`、(a) では Action 側で `close()` する |
