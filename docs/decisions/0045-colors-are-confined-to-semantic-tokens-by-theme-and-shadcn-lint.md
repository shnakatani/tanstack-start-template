# ADR-0045: 色は `@theme` と `@shadcn/lint` の 2 層で semantic token に閉じ込める

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0004 (lint ルールの選定基準)、ADR-0006 (行単位の抑制の許容リスト)、ADR-0020 (`no-restyle` の適用範囲)、ADR-0021 (`require-static-classes` と variant 関数の宣言)、ADR-0046 (`eslint` を必須 peer に持つもう 1 つの経路)
...
- `@shadcn/lint` は `@typescript-eslint/parser` を実依存に持つが、oxlint 経由では読まない。この経路の eslint peer は `pnpm-workspace.yaml` の `packageExtensions` で optional にして止める。ただし `eslint` がグラフから消えるわけではない。`eslint-plugin-testing-library` が `@typescript-eslint/utils` 経由で `eslint` を必須 peer に持ち、そちらは止まらない (ADR-0046)
