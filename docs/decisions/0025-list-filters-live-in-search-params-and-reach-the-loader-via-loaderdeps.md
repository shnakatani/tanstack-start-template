# ADR-0025: 一覧の絞り込み条件は URL の search param が持ち、loaderDeps で loader に渡す

- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0018 (ドメイン型は valibot スキーマから導出する)、ADR-0020 (ナビゲーションは Router の Transition に任せる)、ADR-0026 (入力欄と一覧の描画)、ADR-0047 (wrapper のテスト)
...
- `src/routes/notes/index.tsx` は `Route` と、export しない wrapper だけを持つ (ADR-0015)。入力欄と一覧の描画は ADR-0026 が持つ。テストは ADR-0047、サーバ側の LIKE は `src/server/db/like-pattern.ts`
