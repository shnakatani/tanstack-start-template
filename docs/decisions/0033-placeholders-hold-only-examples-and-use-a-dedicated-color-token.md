# ADR-0033: placeholder には例示だけを置き、色を専用トークンへ切る

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0032 (トークンの値の決め方、`--muted-foreground` を下げた判断、実描画と axe による検算。検算は `::placeholder` には届かない) / ADR-0026 (乖離の記録先)

## Context

ADR-0032「値は palette の段に乗せる」に従って `--muted-foreground` の light を `mist-600` へ下げた。`--muted` の上で WCAG 2.2 SC 1.4.3 の 4.5:1 を満たすためである。
...
トークンを切る形そのものは ADR-0032「1 つのトークンが用途を兼ねて両立しないときは、狭い側を別トークンへ切る」が持つ。
...
「背景と 4.5:1」と「入力値と 3:1」を両方課したときに成立する帯と、palette の段 (ADR-0032「値は palette の段に乗せる」) の比。2026-09-21 の実測である。`src/styles.css` がトークンとして宣言している段は `mise run contrast` で測り直せる (ADR-0036)。light の `mist-500` は `--placeholder`、`mist-600` は `--muted-foreground`。dark の `mist-500` は `--placeholder`、`mist-400` は `--muted-foreground` が持つ。light の `mist-400` と dark の `mist-600` はトークンになっていないので、この手段では測れない。
