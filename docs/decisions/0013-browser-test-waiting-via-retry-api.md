...
- Status: Accepted
- Date: 2026-09-22
- 関連: ADR-0006 (registry コードのガードはブラウザテストが担う)、ADR-0018 (animation を無効にして走らせる)、ADR-0029 (同期読みを assert へ流さない。本 ADR の規範を lint で強制する)、ADR-0030 (assert の予算。`findElement()` を呼ばない理由もここが持つ)、ADR-0031 (否定 assert が不在でも通ること)
...
| 場面                                       | 使うもの                                                                                                                                   |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 操作の結果として現れる要素の mount         | `await expect.element(locator).toBeInTheDocument()`。`findElement()` は呼ばない (ADR-0030)                                                 |
| 操作後の要素の実測 (rect / computed style) | 先に `expect.element` で mount を待ってから `locator.element()` を読む。retry が要るなら `expect.poll` のコールバックの中で読む (ADR-0029) |
...
animation は ADR-0018 の既定で止まるので、開く操作のあとは `expect.element(locator).toBeInTheDocument()` で mount を待ち、実測は `expect.poll` の中で読む (ADR-0029)。`getAnimations()` の完了を待つ helper は置かない。

### 検討した選択肢

| 案                                            | 評価                                                                                                                                                                 | 採否     |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| `expect.element` に寄せる                     | vitest が retry 間隔と失敗時の DOM 出力を持つ。待機の実装がテスト側に残らない。生 DOM の mount 待ちも `expect.element` で足り、`findElement()` は呼ばない (ADR-0030) | **採用** |
...
- 生 DOM を読む箇所は「操作を挟んだか」で待ち方が分かれる。判断を誤ってもテストは大半の実行で通るため、レビューで見る。禁じたい形のうち「同期読みの値を assert へ流すこと」は式の構造で表せるので lint が持つ (ADR-0029)。`element()` を操作前の要素と retry コールバックの中に限ることは実行時の履歴で決まるため、レビューで見る
