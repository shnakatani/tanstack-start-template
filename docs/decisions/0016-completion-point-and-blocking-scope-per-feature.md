...
- Status: Accepted
- Date: 2026-09-14
- 関連: ADR-0014 / ADR-0058 (Action 層と Transition。本 ADR はその上で「どこまで待つか」「何を止めるか」を決める)、ADR-0015 (二重発火の検証)

## Context

ADR-0014 は mutation を Action 層の Transition で実行し、pending を Transition から取ると決める。
...
- ADR-0014 の Decision 表「query の再取得」「ダイアログの開閉」行と、ADR-0058「mutation の書き方」の「再取得と close」行は本 ADR の軸に従う
