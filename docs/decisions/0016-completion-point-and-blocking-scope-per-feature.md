...
- Status: Proposed
- Date: 2026-09-14
- 関連: ADR-0014 (PR #16 で追加。Action 層と Transition。本 ADR はその上で「どこまで待つか」「何を止めるか」を決める)、ADR-0015 (二重発火の検証)
...
| TanStack Query「Invalidations from Mutations」 | `onSuccess` が Promise を返すと再取得完了まで `isPending` が true。返さなければ fire-and-forget                                                                                                                                                                                                                                                                                                                 |
...
| 楽観表示の置き場 | query が持つデータ → TanStack 側 (`variables` / `useMutationState`、複数箇所なら `onMutate` + rollback) / React だけの状態 → `useOptimistic` | ADR-0014「楽観表示の使い分け」と同じ。`useOptimistic` に query の `data` を渡さない (TanStack/query #9742)                                                                                                     |
| ブロック範囲     | 対象の項目だけ / 画面全体                                                                                                                    | 既定は対象の項目だけ。pending は mutation ごとに追う (`useMutationState` で `mutationKey` を絞る)。画面全体を止めるのは並行操作が整合を壊すときだけで、理由を実装近傍に書く                                    |

完了点の軸が決めるのは「ダイアログを閉じる (入力を手放す) 時点」であり、mutation の pending の長さではない。
...
| 完了点 | 閉じ方                                                                                                                                                                                                                               | Transition の pending                                     |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| (a)    | Action は `handle.close()` だけを含み、mutation は Transition の外で走らせる (`void runAction(...)` で捨てる。ADR-0014 の Action の順序保証と完了の観測はこの mutation には効かない)。失敗は `onError` の toast と項目の復帰で伝える | 確定操作で終わる                                          |
| (b)    | `onSuccess` の先頭で `handle.close()` を呼び、その後に再取得の Promise を返す                                                                                                                                                        | 再取得完了まで続くが、閉じた後は見えない                  |
...
| 全機能で (c) 再取得完了まで待つ (ADR-0014 実装時の形) | 再取得が遅い環境でダイアログが固まる。React の非ブロッキングの設計と React Router の pending UI の形 (busy は出すが操作は止めない) から外れる | 却下     |
