...
半透明は `opacity-60` (`src/components/parts/data-table.tsx` の `BUSY_ROW_CLASS`) を使う。`opacity-50` は本文テキストのコントラストを 3.82:1 まで落として WCAG 1.4.3 の 4.5:1 を割る (2026-09-14 に `src/routes/notes/index.test.tsx` の楽観行の a11y 検査が axe で実測した値)。半透明と `aria-busy` は読み上げに出ないため、通知は announcer で出し、行には仮想カーソル用の静的テキスト (「削除中」「保存中」) を置く (ADR-0017)。
