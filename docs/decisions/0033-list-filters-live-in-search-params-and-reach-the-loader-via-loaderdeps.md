...
| loader が読む search は `loaderDeps: ({ search }) => ({ q: search.q })` で宣言し、`loadNotesPageData({ context, deps })` (`-lib/notes-page-loader.ts`) が `notesQueryOptions(deps)` を温める                 | deps に無い search は loader に届かず、preload が別条件のデータを表示側に残す (Router docs の page 2 の例)                                                                                                                                                       |
...
| Route hooks は route ファイル内の wrapper (`NotesRoute`) が吸収し、ページ本体 `NotesPage` (`-components/notes-page.tsx`) は値 (`q`) とハンドラ (`onQueryChange` / `onResultsSettled`) を props で受ける      | Route hooks を混ぜるとページテストがテスト router で動かない (`.claude/rules/directory-structure.md`「ルートファイル」)                                                                                                                                          |
...
- 通知の分担 (ページが報告し wrapper が出す) とその根拠は ADR-0034 が持つ。テストは `-components/notes-page.test.tsx` が `onResultsSettled` の呼び出しを、`route.test.tsx` が `readAnnouncements()` を見る
