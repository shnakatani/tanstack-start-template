...
| 入力値は緊急更新。debounce するのは編集 `{ base, text }` そのもので、debounce 済みの編集も `base === q` のときだけ使い、違えば URL の `q` を条件にする。条件は `useDeferredValue` → `useSuspenseQuery` の順に通す                                                                                                                   | debounce が無いと打鍵ごとに取得する。文字列を debounce すると、確定や戻るで URL が変わった後も debounce 済みの古い値が生き残り、URL でも入力でもない第 3 の条件を描く。`useDeferredValue` が無いと新しい key で Suspend した瞬間に Suspense が古い一覧を隠す                                                |
