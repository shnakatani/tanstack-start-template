...
| Route hooks は route ファイル内の wrapper (`NotesRoute`) が吸収し、ページ本体 `NotesPage` は値 (`q`) とハンドラ (`onQueryChange` / `onResultsSettled`) を props で受ける named export                                | Route hooks を混ぜるとページテストがテスト router で動かない (`.claude/rules/directory-structure.md`「ルートファイル」)                                                                                                                                                  |
...
| 入力欄の値は key にする前に `toNoteListFilter` (`src/routes/notes/-lib/note-search.ts`) で URL と同じ正規化 (trim / 上限で切る) を通し、submit でも同じ値を入力欄と URL に書く。入力欄は `maxLength` を持つ          | trim しないと `" abc"` と `"abc"` が別のキャッシュになる。上限超えを schema に通すと `useSuspenseQuery` が throw し、一覧ごと Error Boundary に落ちる。IME の変換中は maxLength が効かず (facebook/react#8683)、確定の仕方では確定後も超える (Chromium 40520211)         |
| ページは条件の一覧が取得済みになるたび `onResultsSettled(q, count)` で報告し、wrapper が直前に通知した条件と違うときだけ `announce()` で件数を通知する (`noteSearchResultMessage`)。初期表示 (URL の q) は通知しない | `aria-busy` と半透明は読み上げに出ない (ADR-0017 の検討案の表が `aria-busy` 単独の通知を却下)。「最後に通知した条件」をページに持たせると、`key={q}` の作り直し (debounce 前の Enter、戻る) で通知が消える。取得中に報告すると無効化済みキャッシュの古い件数を読み上げる |
| 入力と表示中の条件がずれている間 (正規化後の `text` と `deferredText` が違う。debounce の待ちと取得中) は一覧を `StaleContent` で包み `aria-busy` + 半透明で残す                                                     | 古い一覧が新しい条件の結果に見える。React docs の `isStale` の形。生の文字列で比べると、submit で入力欄を揃えた直後に条件が同じまま印が出る                                                                                                                              |
...
| サーバ側は `title LIKE ? ESCAPE ?` で、bind する値は `%` + `escapeLikePattern(q)` + `%` と `LIKE_ESCAPE_CHAR` の 2 つ。`escapeLikePattern` (`src/server/db/like-pattern.ts`) が `%` `_` `\` をエスケープする         | 利用者の `%` がワイルドカードとして効き、`_` が任意の 1 文字に一致する                                                                                                                                                                                                   |
...
- debounce のテストは 1 文字ずつ別の `userEvent.keyboard` で打つ。`fill` は 1 回の input、`type("abc")` は 3 文字を間を置かず送るので、どちらも debounce の欠落を検出しない (2026-09-23 に mutant で実測)。待ちの実値 (`NOTE_SEARCH_DEBOUNCE_MS`) は `mise run verify` の負荷で打鍵の間隔に負けるので、テストは module の partial mock で待ちを広げる (`index.test.tsx` の `vi.mock`)
...
- 通知の分担は 2 層。ページのテスト (`index.test.tsx`) は `onResultsSettled` の呼び出し (条件と件数、取得中は報告しない) を見て、wrapper のテスト (`route.test.tsx`) が `readAnnouncements()` で Enter と戻るの通知を見る。件数の通知は WCAG 2.2 の 4.1.3 (Status Messages) の「x results returned」の形で、GOV.UK の accessible-autocomplete も常時 mount の `role="status"` を debounce 付きで更新する。「最後に通知した条件」を URL の変化をまたいで残す場所が route component しか無いので、ページが報告し wrapper が通知する
- 取得の決着は `isFetching` が false で見る。オフラインで `fetchStatus` が `paused` のときは古いキャッシュの件数を通知し、再接続後の再取得で件数が変わっても同じ条件なので通知し直さない。表示中の一覧と件数は一致しているので誤通知ではなく、`idle` で見ると通知ゼロになるほうが悪い
...
- WCAG 2.2 Understanding 4.1.3 Status Messages (「x results returned」の例): <https://www.w3.org/WAI/WCAG22/Understanding/status-messages.html>
- GOV.UK accessible-autocomplete `src/status.js` (常時 mount の `role="status"` を debounce 付きで更新): <https://github.com/alphagov/accessible-autocomplete/blob/main/src/status.js>
