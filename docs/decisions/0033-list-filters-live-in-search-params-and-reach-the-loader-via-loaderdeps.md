...
| 入力欄の値は key にする前に `toNoteListFilter` (`src/routes/notes/-lib/note-search.ts`) で URL と同じ正規化 (trim / 上限で切る) を通し、submit でも同じ値を入力欄と URL に書く。入力欄は `maxLength` を持つ          | trim しないと `" abc"` と `"abc"` が別のキャッシュになる。上限超えを schema に通すと `useSuspenseQuery` が throw し、一覧ごと Error Boundary に落ちる。HTML Standard は maxlength の防止を may (任意) としており、効かない経路がある                                     |
...
- 上限で切り詰めたときは `console.warn` に生の入力を残す (`toNoteListFilter`)。切った位置がサロゲートペアの途中なら前半を落とす。落とさないと URL では U+FFFD に化け、`LIKE` にも当たらない (2026-09-23 に better-sqlite3 で実測)
- 通知の分担は 2 層。ページのテスト (`index.test.tsx`) は `onResultsSettled` の呼び出し (条件と件数、取得中は報告しない) を見て、wrapper のテスト (`route.test.tsx`) が `readAnnouncements()` で Enter と戻るの通知を見る
