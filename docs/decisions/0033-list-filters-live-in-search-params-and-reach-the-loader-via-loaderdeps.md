...
- Status: Accepted
- Date: 2026-09-23
- 関連: ADR-0008 (ドメイン型は valibot スキーマから導出する)、ADR-0014 (ナビゲーションは Router の Transition に任せる)、ADR-0035 (入力欄と一覧の描画)、ADR-0036 (wrapper のテスト)、ADR-0037 (サーバ側の LIKE)
...
| loader が読む search は `loaderDeps: ({ search }) => ({ q: search.q })` で宣言し、`loader` は options に直接書いて `notesQueryOptions(deps)` を温める。検証は router 経由 (ADR-0036) | deps に無い search は loader に届かず、preload が別条件のデータを表示側に残す (Router docs の page 2 の例)。loader を関数に切り出すと引数の型を手で書くことになる (`typeof Route` は循環、`LoaderFnContext` は `AnyRoute` 経由で `any`)                                              |
...
- `src/routes/notes/index.tsx` は `Route` と、export しない wrapper だけを持つ (ADR-0012)。入力欄と一覧の描画は ADR-0035、テストは ADR-0036、サーバ側の LIKE は ADR-0037 が持つ
