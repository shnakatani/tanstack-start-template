...
- debounce のテストは 1 文字ずつ別の `userEvent.keyboard` で打つ。`fill` は 1 回の input、`type("abc")` は 3 文字を間を置かず送るので、どちらも debounce の欠落を検出しない (2026-09-23 に mutant で実測)。待ちの実値 (300ms) は `mise run verify` の負荷で打鍵の間隔に負けるので、テストは module の partial mock で待ちを広げる (`index.test.tsx` の `vi.mock`)
