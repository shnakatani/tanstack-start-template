...
| 入力欄の値は入力の直後に 1 回だけ `toNoteListFilter` (`src/routes/notes/-lib/note-search.ts`) で URL と同じ正規化 (trim / 上限で切る) を通し、debounce / deferred / key / submit はその値から導く。切り詰めは汎用の `truncateCodeUnits` (`src/lib/truncate-code-units.ts`)。入力欄は `maxLength` を持つ                             | trim しないと `" abc"` と `"abc"` が別のキャッシュになる。上限超えを schema に通すと `useSuspenseQuery` が throw し、一覧ごと Error Boundary に落ちる。正規化を下流の複数箇所で呼ぶと、呼び忘れた経路が生の値で走る。IME の変換中は maxLength が効かず (facebook/react#8683)、確定の仕方では確定後も超える (Chromium 40520211) |
