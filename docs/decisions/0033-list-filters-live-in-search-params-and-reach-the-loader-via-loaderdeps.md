...
| 入力欄の state は「URL の `q` に対する編集」`{ base: q, text }` で持ち、表示値は `base === q` のときだけ `text`、それ以外は `q` (描画中に導く)。`key={q}` でページを作り直さない                             | 作り直すと確定 (Enter) のたびに入力欄が新しい要素になりフォーカスが body へ落ちる (2026-09-23 に実測)。テーブルやダイアログの state も捨てる。effect で setState すると 1 描画ぶん古い値が見える                                                                 |
| 入力値 `text` は緊急更新。一覧は `text` が URL の `q` と同じならそのまま、違えば `useDebouncedValue(text, { wait: NOTE_SEARCH_DEBOUNCE_MS })` を通し、`useDeferredValue` → `useSuspenseQuery` の順に通す     | debounce が無いと打鍵ごとに取得する。確定と戻るの直後に debounce を待つと、loader が温めた条件を 300ms 遅れて表示する。`useDeferredValue` が無いと新しい key で Suspend した瞬間に Suspense が古い一覧を隠す                                                     |
