...
| 結果の入れ替わりの通知はページが `onResultsSettled(q, count)` で決着を報告し、wrapper が直前の条件と比べて `announce()` する (ADR-0034)                                                                      | `aria-busy` と半透明は読み上げに出ない。分担の理由と先行例は ADR-0034 が持つ                                                                                                                                                                                     |
...
- 通知の分担 (ページが報告し wrapper が出す) とその根拠は ADR-0034 が持つ。テストは `index.test.tsx` が `onResultsSettled` の呼び出しを、`route.test.tsx` が `readAnnouncements()` を見る
