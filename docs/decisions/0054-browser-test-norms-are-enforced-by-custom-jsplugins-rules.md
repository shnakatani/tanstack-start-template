...
| 期待値がリテラルの否定 (`not.toHaveStyle(...)` / `getComputedStyle` 起点の `not.toBe(...)`) | 期待値の綴りが 1 つ外れると否定が真になり、潰れた状態でも通る                                       |
...
- 導入は 1 つの PR で全違反を移し終える。未移行ファイルを `lint.overrides` で列挙しない
...
| severity を `warn` にして移行を待つ                                                             | `vp check` は warn で exit 1 にならず、新規コードへの強制力を失う (`docs/guides/lint.md`「testing-library を当てる範囲」の `no-debugging-utils` と同じ判断)                 | 却下     |
...
| `expectAbsent` の anchor を引数で必須にする (`expectAbsent(x, { after: anchor })`)              | 型で止まるのは anchor の書き忘れだけで、anchor が同じ操作の効果を表すかは変わらずレビューで見る。12 箇所中 2 箇所は locator の存在で表せず thunk が要り、強制が形だけになる | 却下     |
...
- 導入時の違反は、`prefer-locator-methods` が 54 件 (1 行の grep で数えた 45 件 / 13 ファイルに、変数へ束縛してから渡す 9 件)、`no-find-element` が 0 件 (移行前の 2026-09-22 には 16 箇所)、`no-negated-style-literal` が 1 件 (テンプレート初版 `fb3433a` からの `dialog-scroll-body.test.tsx`)。すべて導入と同じ変更で移した
