# ADR-0040: axe の incomplete は描画を統制できる層でだけ落とし、緑を測った証明とせず、抑制は出た story に理由と出口を添えて置く

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0046 (incomplete に噛まれた事故と回避策) / ADR-0037 (a11y 検査の対象) / ADR-0057 (story を検査の単位にする) / ADR-0034 (1.4.11 を axe が持たない) / ADR-0036 (`::placeholder` を axe が誤って評価する) / ADR-0009 (静的 lint の構成)
...
後者は 2026-09-21 時点で緑だが、それはそのテストがたまたま `incomplete` を出さないためである。**この基準には既に噛まれている。** ADR-0046 は、確認ダイアログを閉じた直後の検査が Base UI の focus guard を `aria-hidden-focus` の `incomplete` として拾い、CI でだけ落ちた事故の記録である。そのとき基準を見直さず、animation を無効にする回避策を足して緑へ戻した。
...
ブラウザテストで落とさないのは、**そこで出るものが部品の問題ではなく、実行環境の速さで結果が変わるからである。** ADR-0046 はその記録で、確定でダイアログを閉じた直後の検査が閉じかけの popup を拾い、CI でだけ落ちた。同 ADR は「検査がこの窓の内側に落ちるか外側に落ちるかは実行環境の速さで決まり、遅い CI ほど内側に落ちる」と書いている。直しようのないものをエラーにしたので、animation を無効にする回避策が要った。**基準が逆だったから回避策が生まれた。**
...
| 1.4.11 (非テキスト 3:1) | **0 ルール**              | トークンの値を人が測る (ADR-0034)      |
| 1.4.1 (色の使用)        | `link-in-text-block` だけ | 本文中のリンク以外は見ない             |
| `::placeholder`         | 誤った前景色で評価する    | 人が見比べる (ADR-0036、axe-core#4260) |
...
境界は「操作の前か後か」ではない。story も `play` で操作の後の状態を見る (ADR-0053)。分かれるのは**置ける場所**と、そこから来る**描画を統制できるか** (上の「`incomplete` を落とすかは、描画を統制できる層かで決める」) である。`.storybook/main.ts` の `stories` は `src/components/**` しか見ないので、ページと文書全体は story にできない。`root-document.test.ts` が見ているランドマーク構造は部品へ分解できず、ブラウザテストでしか押さえられない。両方要る。
...
| 全ルールの `incomplete` を入れる                | 却下 | 混成のバケツを区別せず、ADR-0046 の事故を全 story へ広げる                                                                                   |
...
- `expectNoA11yViolations` は `incomplete` を見ない。ADR-0046 の animation 無効化は、`incomplete` を落とす基準に対する回避策として置かれている。`incomplete` を見ない基準の下で、その回避策が他の理由 (実イベントの規律、ADR-0044 / ADR-0045) でも要るかは別に確かめる
