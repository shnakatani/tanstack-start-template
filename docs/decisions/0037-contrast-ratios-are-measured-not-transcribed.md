# ADR-0037: コントラスト比は測り方を実在させ、動く数値を文書へ書き写さない

- Status: Accepted
- Date: 2026-09-22
- 関連: ADR-0033 (比の値と計測方法の置き場) / ADR-0034 (帯の表) / ADR-0039 (axe が測らない範囲) / ADR-0001 (数値の書き方) / ADR-0023 (半透明の行の比の出所)
...
4 件目は ADR-0023 に残っていた。`opacity-50` の比を ADR-0023 と `data-table.tsx` が別の値で書いており、同じ主張を 2 箇所へ写したことが原因である。
