...
| button.tsx                    | cva base の `disabled:` の対を `data-disabled:` の対へ置換                                                                                   | Base UI は `disabled` prop から `data-disabled` を必ず出し、native `disabled` は `focusableWhenDisabled` のとき付けない。`disabled:` の適用範囲は `data-disabled:` に含まれる (`@base-ui/react` 1.8.0 の `getStateAttributesProps.js`)                 | 上流が同じ対応を入れたら撤去。現時点で該当 issue は無い。ガードは `button.test.tsx`                                                                 |
