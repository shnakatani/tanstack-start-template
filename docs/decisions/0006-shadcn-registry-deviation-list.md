...
| item.tsx                      | `ItemGroup` を `useRender` にして `render` を通す                                                                                            | 既定の `role="list"` の div は子に `role="listitem"` を要求するが、`<li>` は ul/ol/menu の中でしか置けず HTML が破綻する。`render={<ul />}` と `Item render={<li />}` の対で使えば ARIA も content model も揃う                                                                                                                                                                                                             | 上流が list の意味をネイティブのタグで出したら撤去                                                                                                  |
