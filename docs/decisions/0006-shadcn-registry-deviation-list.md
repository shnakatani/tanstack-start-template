...
| button.tsx                    | cva base へ `data-disabled:pointer-events-none data-disabled:opacity-50` を追加                                                              | Base UI は `focusableWhenDisabled` のとき native `disabled` を付けず `aria-disabled` にするため、registry の `disabled:` variant が構造的に当たらない (`@base-ui/react` 1.8.0 の `utils/useFocusableWhenDisabled.js`)                                  | 上流が同じ対応を入れたら撤去。現時点で該当 issue は無い。ガードは `button.test.tsx`                                                                 |
