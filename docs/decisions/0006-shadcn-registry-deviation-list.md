...
| `--success`                                                | 生成物に無い。light `green-800` / dark `green-500` を足し、`@theme inline` へ `--color-success` を通す                                                                                   | 上流は成功のトークンを持たない。アイコンと本文の両方へ使うため、light dark とも 4.5:1 を満たす段を採る。`--background` の上で緑の既定に近い `green-600` は 3.22 で届かず、`green-800` は 7.09、dark の `green-500` は 8.87 | 上流が成功のトークンを持ったら撤去。消費者は `toast.tsx` の成功アイコン                          |
