...
| combobox.tsx                  | `ComboboxContent` の props でアクセシブル名を型必須にする (`aria-label` か `aria-labelledby`)                                                | popup 内に入力欄を置く構成では base-ui が popup へ `role="dialog"` を付ける。dialog は名前が要るが、正しい名前は消費側しか知らないので既定値は持てない。型で要求すれば書き忘れが `vp check` で落ちる                                                   | 上流が名前を要求するか自動で張るようになったら撤去                                                                                                  |
...
| item.tsx                      | `ItemSeparator` を base-ui の `Separator` から外し、装飾の線として描く                                                                       | `Separator` は `role="separator"` と `aria-orientation` を付けるため list の子に置けず、`role="presentation"` へ倒しても `aria-allowed-attr` で落ちる。対で export される部品どうしが組めない                                                          | 上流が list の中で使える形にしたら撤去。ガードは `item.stories.tsx` の Grouped                                                                      |
| progress.tsx                  | `Progress` の props で、`children` を渡さないならアクセシブル名を型必須にする                                                                | `ProgressLabel` を置けば base-ui が `aria-labelledby` を張るが、省略した形は名前を持たない。`<Progress value={60} />` が最短の書き方なので、そこが必ず axe に落ちる                                                                                    | 上流が名前を要求するようになったら撤去                                                                                                              |
