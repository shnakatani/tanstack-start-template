...
| input.tsx                     | `placeholder:text-muted-foreground` を `placeholder:text-placeholder` へ                                                                     | 例示テキストを入力値と区別が付く段に留める。`--muted-foreground` は `--muted` の上の 4.5:1 のために下げてあり、兼ねると例示が入力済みに見える (ADR-0025)                                                                                                                                                                                                                                                                    | 上流が例示専用のトークンを持ったら撤去。この色を見るガードは無い (ADR-0025)                                                                         |
...
| `--placeholder`                                            | 生成物に無い。light dark とも `mist-500` を足し、`@theme inline` へ `--color-placeholder` を通す。light の値は生成物の `--muted-foreground` と同じ  | `::placeholder` の例示テキスト専用に切った。`--muted-foreground` を `mist-600` へ下げると例示が入力値と紛れる。`select` の `data-placeholder:` は実テキストなので対象外 (ADR-0025)                                         | 上流が例示専用のトークンを持ったら撤去。この色を見るガードは無い (ADR-0025)                      |
