# ADR-0011: ルールの選定は上流 recommended を基準にし、typescript だけ strict を基準にする

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0010 (プラグインの設定方法)、ADR-0018 (React Compiler の診断ルールの扱い)、ADR-0031 (色の統制に足す `@shadcn/lint`)、ADR-0012 (testing-library の適用範囲)、ADR-0013 (テスト専用コードの import 境界)、ADR-0020 (`no-misused-promises` が要求するハンドラの形)
...
`@shadcn/lint` はこの表に載らない。oxlint ネイティブではなく `jsPlugins` 経由で、基準も recommended ではなく設計判断と対にしたルールを名指しするためである (ADR-0031)。

`testing-library` もこの表に載らない。基準は上流の `flat/react` を写すが、適用を story と story 専用の helper に限り、基準から外すルールがある (ADR-0012)。
...
| `no-restricted-imports`                 | `*.test-helpers.ts` と `src/test/` をアプリのコードから import させない (ADR-0013) |

テスト専用コードの import を止める範囲と手段の比較は ADR-0013 が持つ。
...
範囲を絞って有効にするルール (`no-restricted-imports`) は緩和ではないので、この経路に載せず `excludeFiles` で対象を外す (ADR-0013)。
