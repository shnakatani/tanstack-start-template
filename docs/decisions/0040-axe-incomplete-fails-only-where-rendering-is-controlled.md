# ADR-0040: axe の incomplete は描画を統制できる層でだけ落とし、`color-contrast` の incomplete は外さない

- Status: Accepted
- Date: 2026-09-21
- 関連: ADR-0041 (緑を測った証明としない) / ADR-0042 (層ごとの役割と抑制の置き場所) / ADR-0043 (a11y の検査を tag で分ける) / ADR-0046 (incomplete に噛まれた事故と回避策) / ADR-0037 (a11y 検査の対象) / ADR-0057 (story を検査の単位にする)
