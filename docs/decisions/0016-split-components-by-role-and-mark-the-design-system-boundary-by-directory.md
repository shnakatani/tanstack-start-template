# ADR-0016: `src/components/` を役割で分け、design system の著作と消費の境界をディレクトリで表す

- Status: Accepted
- Date: 2026-09-19
- 関連: ADR-0032 (`@shadcn/lint` のルールの選定)、ADR-0027 (registry コードの改変は許容リストで統制)、ADR-0015 (features / routes の配置の原則)、ADR-0022 (Action 層)、ADR-0024 (DataTable 部品の配置)
...
| `src/components/action/`  | `ui/` を包み `action` prop で Transition 化した部品 (ADR-0022) | 適用外       |
...
- registry を包んでドメイン固有の外見を定義する部品の置き場所は未定義。現時点で該当は無く、出てきた時点で `parts/` に置くか `src/features/<domain>/` 側の扱いとするかを決める (ADR-0015)
