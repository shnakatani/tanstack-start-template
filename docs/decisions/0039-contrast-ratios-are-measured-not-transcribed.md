...
Primer が数値を書き写さない形に到達していることと、ADR-0001 が、コードで動く数値は書かず、測り方だけ残すと定めていることは一致する。
...
判定軸は ADR-0001 の数値の扱いである。コードで動く数値は書かず、測り方だけ残す。外部の一時点の観測と判断の根拠は日付つきで書く。これをコントラスト比へ当てる。
