...
- Status: Accepted
- Date: 2026-09-22
- 関連: ADR-0034 (比の値と計測方法の置き場) / ADR-0036 (帯の表) / ADR-0041 (axe が測らない範囲) / ADR-0001 (数値の書き方) / ADR-0021 (半透明の行の比の出所)
...
Primer が数値を書き写さない形に到達していることと、ADR-0001 が、コードで動く数値は書かず測り方だけ残すと定めていることは一致する。
...
判定軸は ADR-0001 の数値の扱いである。リポジトリのコードや作業で動く数値は書かず測り方だけ残し、判断の根拠を兼ねる数値は日付を添えて残す。これをコントラスト比へ当てる。
