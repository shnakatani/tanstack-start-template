...
| matcher の無い実測 (rect / computed style / `matches()`) は `expect.poll` のコールバックの中で読む。helper に閉じた読み (`src/test/viewport.ts` 等) は呼び出し側が mount を `expect.element` で待ってから呼ぶ (ADR-0013) | 直接 `expect()` へ流す形はルールが止める。helper 越しの読みはルールが追わない (「ルールが追えない形」) ので、待ちの順序はレビューで見る |
