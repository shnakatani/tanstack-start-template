...
| [0014](0014-transitions-by-default-for-user-updates.md)   | ユーザー操作による更新は Transition を既定にし、pending は Transition から取る               | Proposed | 2026-09-13                    | mutation は `src/components/action/` の `action` prop から `useActionMutation` の `runAction` を呼ぶ。`onSuccess` は再取得を await してから close し、楽観表示は mutation の `variables` を使う (`useOptimistic` は query 外のみ) |
