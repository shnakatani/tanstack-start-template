...
| [0016](0016-completion-point-and-blocking-scope-per-feature.md) | ユーザー操作の完了点とブロック範囲は機能ごとに選び、既定は対象の項目だけを止める               | Proposed | 2026-09-14                    | 完了点 (楽観 / サーバー応答 / 再取得完了)、楽観表示の置き場、ブロック範囲の 3 軸で機能ごとに選ぶ。既定は失敗時に戻せるなら楽観、戻せないならサーバー応答で、止めるのは対象の項目だけ。mutation は `onSuccess` で再取得の Promise を返して pending を保ち、対象の項目がそこから busy を表現する |
| [0017](0017-live-region-announcer-for-status.md)                | 状態の通知は常時 mount の live region に集約し、項目の状態は静的テキストと `aria-busy` で持つ  | Proposed | 2026-09-14                    | root に polite / assertive の region を 1 組常時 mount し `announce()` で更新する。項目 (行・ボタン) は `aria-busy` と静的テキストで状態を持ち、live region を条件付きで mount しない。`Spinner` は視覚専用                                                                                    |
| [0018](0018-browser-tests-disable-base-ui-animations.md)        | ブラウザテストは Base UI の animation を無効にして走らせ、animate-out の窓を踏むテストだけ戻す | Proposed | 2026-09-14                    | `browser-setup.tsx` が毎テスト `BASE_UI_ANIMATIONS_DISABLED` を立て、閉じかけの popup が残る窓を消す。窓を検証するテストは `enableBaseUiAnimations()` で戻す。popup を閉じた後の axe は unmount を待つ                                                                                         |
