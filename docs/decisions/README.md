...
| [0004](0004-lint-rule-selection-criteria.md)        | ルールの選定は上流 recommended を基準にし、typescript だけ strict を基準にする               | Accepted | 2026-08-17（2026-09-07 改訂） | カテゴリ有効化は `correctness` と `perf` に限る。off の条件、テストの緩和 5 ルール、jsx-a11y の名指しゼロ、tailwind と TanStack の領域を `jsPlugins` で足す判断もここが持つ                             |
