...
| [0004](0004-lint-rule-selection-criteria.md)                    | ルールの選定は上流 recommended を基準にし、typescript だけ strict を基準にする               | Accepted | 2026-08-17（2026-09-07 改訂） | カテゴリ有効化は `correctness` と `perf` に限る。off の条件、テストの緩和 5 ルール、jsx-a11y の名指しゼロ、tailwind 領域を `jsPlugins` で足す判断もここが持つ                                                                                                                                  |
...
| [0014](0014-transitions-by-default-for-user-updates.md)         | ユーザー操作による更新は Transition を既定にし、pending は Transition から取る               | Accepted | 2026-09-13                    | mutation は `src/components/action/` の `action` prop から `useActionMutation` の `runAction` を呼ぶ。`onSuccess` は再取得を await してから close し、楽観表示は mutation の `variables` を使う (`useOptimistic` は query 外のみ)                                                              |
