...
| [0019](0019-list-tables-via-tanstack-table-column-definitions.md) | 一覧テーブルは TanStack Table v9 の列定義で組み、描画は registry の Table に残す               | Proposed | 2026-09-14                    | `createColumnHelper` の列定義を SSOT にし、shadcn Data Table と同じ `DataTable` 部品が registry の `Table` に `FlexRender` で描く。楽観行は行の union で同じ列定義に載せる。楽観表示の設計は ADR-0016 のまま                                                                                   |
