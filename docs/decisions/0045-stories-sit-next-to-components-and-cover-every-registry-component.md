...
`*.stories.tsx` は部品と同じディレクトリに置く。`src/components/ui/` に置いたものも `*.test.tsx` と同じ「registry 由来でない付随ファイル」として baseline 検査の対象外になる (ADR-0024)。
...
- `.stories.tsx` を registry のディレクトリに置くため、ADR-0024 の baseline 検査に除外が 1 種類増える
