# ADR-0030: 消費側の className は静的に読める形に保ち、共有する外見は部品で配る

- Status: Accepted
- Date: 2026-09-19
- 関連: ADR-0031 (`@shadcn/lint` のルールの選定)、ADR-0021 (Action 層の責務)、ADR-0015 (コンポーネントの層と適用範囲の表し方)

## Context

`@shadcn/lint` で design system の契約を守る系統は `no-restyle` と `require-static-classes` の 2 つある。
前者は ADR-0015 の層の決定と対で先に採り、後者は保留していた (ADR-0031)。
...
色と未知 class の統制 (ADR-0031) の手前にある門番にあたる。
