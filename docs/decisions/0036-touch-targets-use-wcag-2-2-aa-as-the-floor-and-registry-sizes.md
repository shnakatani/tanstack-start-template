# ADR-0036: touch target は WCAG 2.2 AA を床とし registry 素寸法で一本化する

- Status: Accepted
- Date: 2026-08-17
- 関連: ADR-0026 (registry 乖離の許容リスト)
...
`src/**/*.{ts,tsx}` の字面を走査する自前の検査は置かない。書いてよい場合 (誤タップレバー) との区別が variant の有無という字面にしか現れず、走査側が例外を抱え続ける構造になるためである。
素の `w-11` はこの禁止の対象に含めない。横 44px の焼き込みは `min-w-11` で書く決まりで、素の `w-11` はアイコン枠などの固定幅として正当な用途がある。

本 ADR の決定のうち機械で見るのは、ポインタが本体へ届かなくなる壊れ方だけである。範囲は `src/components/ui/touch-target.test.tsx` を読む。
寸法は機械で見ない。消費側が size を変えるのは正当な使い方で、値を焼き付けると最初の消費者が消すテストになるうえ、上流が寸法を変えただけで落ちる。床も 44px の焼き込みもレビューで見る。
...
- 素寸法で運用する以上、registry の更新で寸法が変わればこちらの実寸も変わる。ADR-0025 の baseline diff がその変化を可視化する
