# ADR-0013: テスト専用コードの import は `no-restricted-imports` で止める

- Status: Accepted
- Date: 2026-09-20
- 関連: ADR-0011 (lint ルールの選定基準と「基準から外れる名指し」、テストファイルの緩和)
...
全体で error にしてテスト側で off にする形は取らない。off はテストの緩和経路に載り、`*.test-helpers` を緩和へ足すことになる (ADR-0011「テストファイルの緩和」の 5 ルールは helper に要らない)。
