# ADR-0043: ブラウザテストは animation を無効にして走らせ、animate-out の窓を踏むテストだけ戻す

- Status: Accepted
- Date: 2026-09-22
- 関連: ADR-0041 (待機は vitest の retry API に委ねる)、ADR-0042 (二重発火の検証は実イベントで書く)、ADR-0035 (a11y 検査の対象)
...
- 二重発火の dedupe テスト (ADR-0042) は animate-out の窓を踏む必要があるため、animation を戻して走らせる (書き方は `docs/guides/testing.md`「animation を戻すテストを書く」)
