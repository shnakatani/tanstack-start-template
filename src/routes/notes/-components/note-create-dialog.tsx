...
export function NoteCreateDialog() {
  ...
  const createMutation = useActionMutation({
    ...
    // 完了点 (b): 応答で閉じ、再取得を await して pending を再取得完了まで保つ (ADR-0023)。
    ...
    onSuccess: async () => {
      ...
      // 一覧への行の追加は読み上げに出ないので、完了を通知する (ADR-0035)
      announce("保存しました");
    },
  ...
  // だけで、一覧の操作は止めない (ADR-0023「ブロック範囲」)。
  //
  ...
  // ADR-0023 移行前の従来挙動 (何も止めない) と同じなので、閉じられなくなる側へは倒さない
