 ...
 * route ファイルから export せずここに置く (ADR-0014)。入力欄と一覧の流れは ADR-0025、
 * 件数の通知は ADR-0035。
 */
export function NotesPage({ q, onQueryChange }: { q: string; onQueryChange: (q: string) => void }) {
  ...
  // state when a prop changes」。ADR-0025)
  ...
  // 結果の入れ替わりの通知 (ADR-0035)。取得中は古い件数を読むので決着まで待ち、直前と同じ条件なら出さない。
  ...
  // pending な削除の対象 id。mutation ごとに追うので、同時削除でも各行が busy になる (ADR-0022)。
  ...
  // 一覧の先頭に出すこの行だけが伝える (ADR-0022)。mutation はダイアログ側にあるため
  ...
  // 完了点 (a): Action は close だけを含み、mutation は Transition の外で走らせる (ADR-0022)。
        ...
        <StaleContent stale={isStale}>
          {rows.length === 0 ? (
            ...
            <DataTable
              ...
              // busy の判定は行データから (ADR-0022)。通知は announcer が担う (ADR-0034)
              rowProps={({ original }) => {
