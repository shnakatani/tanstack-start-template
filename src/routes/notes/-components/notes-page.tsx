...
export function NotesPage({ q, onQueryChange }: { q: string; onQueryChange: (q: string) => void }) {
  ...
  // 結果の入れ替わりの通知 (ADR-0033)。取得中は古い件数を読むので決着まで待ち、直前と同じ条件なら出さない。
  ...
  const deleteMutation = useActionMutation({
    ...removeNoteMutation,
    // 開始の通知の置き場 (ADR-0032)。この画面は variables 方式 (ADR-0020) なのでキャッシュは触らない。
    ...
    onSuccess: async (_data, target) => {
      ...
      // 行の消失は読み上げに出ないので、完了を通知する (ADR-0032)
        ...
        <StaleContent stale={isStale}>
          {rows.length === 0 ? (
            ...
            <DataTable
              ...
              // busy の判定は行データから (ADR-0020)。通知は announcer が担う (ADR-0032)
              rowProps={({ original }) => {
