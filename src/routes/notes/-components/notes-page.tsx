 ...
 * route ファイルから export せずここに置く (ADR-0012)。入力欄と一覧の流れは ADR-0023。
 */
export function NotesPage({ q, onQueryChange }: { q: string; onQueryChange: (q: string) => void }) {
  ...
  // state when a prop changes」。ADR-0023)
  ...
  // 結果の入れ替わりの通知。announcer (ADR-0030) を、取得の決着を契機にここから呼ぶ。取得中は古い件数を読むので決着まで待ち、直前と同じ条件なら出さない。
  ...
  // pending な削除の対象 id。mutation ごとに追うので、同時削除でも各行が busy になる (ADR-0020)。
  ...
  // 一覧の先頭に出すこの行だけが伝える (ADR-0020)。mutation はダイアログ側にあるため
  ...
  // 完了点 (a): Action は close だけを含み、mutation は Transition の外で走らせる (ADR-0020)。
        ...
        <StaleContent stale={isStale}>
          {rows.length === 0 ? (
            ...
            <DataTable
              ...
              // busy の判定は行データから (ADR-0020)。通知は announcer が担う (ADR-0030)
              rowProps={({ original }) => {
