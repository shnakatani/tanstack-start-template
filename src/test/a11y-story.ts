...
// ブラウザテストからは呼ばない。
//
// 層を分ける軸は ADR-0056。story は描画を統制できるので `incomplete` を落とし、
// 組み上げて操作するブラウザテストでは落とさない。

/**
 * 合否から外す `incomplete`。単一部品でも統制の外へ出る状態だけを、
 * (ルール, `messageKey`) の粒度で挙げる (ADR-0056)。
 *
...
export function checkA11yIncomplete(context: {
  ...
  if (report === undefined) {
    return "addon-a11y のレポートが無い。afterEach の実行順が変わったか、a11y の検査自体が動いていない (ADR-0056)";
  }
  ...
  if (!hasIncompleteResults(report.result)) {
    return "addon-a11y のレポートを読めない。addon の結果の形が変わった (ADR-0056)";
  }
 ...
 * 外さないキーが 1 つでも混ざっていたら、その node は部品側の信号を含んでいる (ADR-0056)。
 *
