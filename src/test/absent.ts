 ...
 * 違いは retry の予算だけなので、名前でどちらのつもりかを表明する (ADR-0046)。
 *
 ...
 * Playwright が公式に警告している形で、その肯定 assert が緩和にあたる (ADR-0046)。
 */
...
/**
 * 要素が在る状態から消えるのを待つ。**assert の予算ぶん待つ** (ADR-0045)。
 *
 * base-ui は `animate-out` の完了まで unmount を遅らせるため、消えるのを待つ側には retry の
 * 予算が要る (ADR-0043)。`expectAbsent` に置き換えると unmount を待たずに落ちる。
 *
 * 要素が最初から無くても通る。前段の操作がその要素を消すものであることは呼び出し側が
 * 担保する。ランタイムでは止められない理由は ADR-0046「不在の 2 つの意味は…」が持つ。
 */
