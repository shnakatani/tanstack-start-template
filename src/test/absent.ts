...
/**
 * 要素が在る状態から消えるのを待つ。**assert の予算ぶん待つ** (docs/guides/testing.md「assert の予算を宣言する」)。
 *
 * base-ui は `animate-out` の完了まで unmount を遅らせるため、消えるのを待つ側には retry の
 * 予算が要る (docs/guides/testing.md「animation を戻すテストを書く」)。`expectAbsent` に置き換えると unmount を待たずに落ちる。
 *
 * 要素が最初から無くても通る。前段の操作がその要素を消すものであることは呼び出し側が
 * 担保する。ランタイムでは止められない理由は docs/guides/testing.md「不在を 2 つの名前で書き分ける理由」が持つ。
 */
