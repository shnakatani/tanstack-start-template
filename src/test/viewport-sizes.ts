/**
 * viewport の寸法。寸法はすべてここに置く。`vitest.browser.config.ts` と
 * `vitest.storybook.config.ts` が `DEFAULT_VIEWPORT` を `browser.viewport` として読むため、
 * このファイルは `vite-plus/test/browser` を import しない。
