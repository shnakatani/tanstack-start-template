 ...
 *
 * 消費者は 2 つ。`vitest.browser.config.ts` が `expect.poll.timeout` と
 * `actionTimeout` に渡し、`src/test/find-element.ts` が `findElement` に渡す。
 * 後者は前者の副作用を補うためで、理由はそちらの docstring が持つ。
 */
