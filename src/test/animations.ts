...
/**
 * ブラウザテストの既定 (docs/guides/testing.md「animation を無効にして走らせる理由」)。`src/test/browser-setup.tsx` の `beforeEach` が毎テスト呼ぶ。
...
/**
 * このテストの間だけ animation を戻す (docs/guides/testing.md「animation を戻すテストを書く」)。閉じかけの popup が残る窓を検証するテストが
