 ...
 * 止まる (ADR-0046)。pending の外見は ActionButtonShell の story が args だけで持つ。
 *
...
export const Settles: Story = {
  ...
  play: async ({ args }) => {
    ...
    // Spinner は視覚専用で accessibility API に出さない (ADR-0032)。ここだけ要素を直に
...
export const NotCalledTwice: Story = {
  ...
  play: async ({ args }) => {
    ...
    // (ADR-0047)。この story が固定するのは、描画された guard が再操作を塞ぐことだけ。
