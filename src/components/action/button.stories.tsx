 ...
 * 止まる (ADR-0048)。pending の外見は ActionButtonShell の story が args だけで持つ。
 *
...
export const Settles: Story = {
  ...
  play: async ({ args }) => {
    ...
    // Spinner は視覚専用で accessibility API に出さない (ADR-0034)。ここだけ要素を直に
...
export const NotCalledTwice: Story = {
  ...
  play: async ({ args }) => {
    ...
    // (ADR-0049)。この story が固定するのは、描画された guard が再操作を塞ぐことだけ。
