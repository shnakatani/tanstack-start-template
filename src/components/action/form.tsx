...
type ActionFormProps = Omit<ComponentProps<"form">, "onSubmit" | "children"> & {
  /** submit で実行する Action。`startTransition` に渡し、決着まで pending になる (ADR-0021) */
  submitAction: () => Promise<void> | void;
  children: ReactNode;
};

/**
 * submit を Transition にする `<form>` (ADR-0021「Action 層」)。`ui/` に対応部品が無い唯一の例外で、
 * 素の `<form>` を包む。pending は子孫の `ActionFormSubmit` が context から読む
 * (React の `<form action>` + `useFormStatus` と同じ形。使わない理由は ADR-0021「Action 層」)。
 */
