...
/**
 * pending を prop で受ける Button の共通部 (ADR-0020「Action 層」)。`ActionButton` と
 ...
 *   (ADR-0020「二重発火は state だけで塞ぐ」、検証方法は ADR-0045)
...
type ActionButtonProps = Omit<ActionButtonShellProps, "isPending" | "onClick"> & {
  /** クリックで実行する Action。`startTransition` に渡し、決着まで pending になる (ADR-0020) */
  action: () => Promise<void> | void;
};

/**
 * `action` prop を受ける Button (ADR-0020「Action 層」)。pending は `useTransition` の `isPending` から取る。
