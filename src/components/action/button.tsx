...
/**
 * pending を prop で受ける Button の共通部 (ADR-0022「Action 層」)。`ActionButton` と
...
type ActionButtonProps = Omit<ActionButtonShellProps, "isPending" | "onClick"> & {
  /** クリックで実行する Action。`startTransition` に渡し、決着まで pending になる (ADR-0022) */
  action: () => Promise<void> | void;
};

/**
 * `action` prop を受ける Button (ADR-0022「Action 層」)。pending は `useTransition` の `isPending` から取る。
