...
/**
 * touch target の AA 基準一本化 (ADR-0007)。視覚 = ヒット = registry 素寸法。
 *
 * 寸法・疑似要素の不在・touch-action の既定は規範と ADR が持ち、機械で見ない
 * (ADR-0007「寸法は機械で見ない」。疑似要素の拡大は registry の sidebar 自身が
 * `after:-inset-2` で持つため、部品ごとの不在確認は一貫した保証にならない)。
 * `type="number"` の spinner は NumberField を採る決定 (ADR-0010) により対象外。
 *
 * ここに残すのは pointer が input 本体に届くこと。Playwright の click は hit-target の検査を
 * 通すので、包む要素や疑似要素が pointer を奪えば click 自体が落ちる (ADR-0015)。
 */
