 ...
 * axe に判定させる (ADR-0034「リポジトリが持つ検算は実描画と axe で行い、比を計算する story を持たない」)。この story は比を計算しない。計算を持たせると、描画では
 ...
 * dark 側を確かめるときは `vp test run` で回す (ADR-0057)。2026-09-21 時点では
 ...
 * 一致するが、axe は画面の面の重なりまで畳むので、渡す面が違えば値も違う (ADR-0039)。
 *
