 ...
 * axe に判定させる (ADR-0032「リポジトリが持つ検算は実描画と axe で行い、比を計算する story を持たない」)。この story は比を計算しない。計算を持たせると、描画では
 * なく計算のほうを検査することになり、story が描いた色と判定がずれても気づけない。比を数で
 * 知りたいときは `mise run contrast` を使う (ADR-0036)。
 *
 ...
 * dark 側を確かめるときは `vp test run` で回す (ADR-0052)。2026-09-21 時点では
 ...
 * 一致するが、axe は画面の面の重なりまで畳むので、渡す面が違えば値も違う (ADR-0036)。
 *
