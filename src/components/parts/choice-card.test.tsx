 ...
 * 行間・disabled の見え方 (cursor / data-disabled) は同 story の状態カタログで見る (ADR-0039)。
...
describe("ChoiceCard", () => {
  ...
  // 下の disabled のテストは click が届かなくても緑になる (ADR-0037)
  ...
  it("disabled の行はクリックしてもトグルせず、押せると主張しない", async () => {
    ...
    // 対象に pointer-events: none が無く click は実際に届くため force で検査だけ飛ばす (ADR-0034)
    await screen.getByText("チームA").click({ force: true });
