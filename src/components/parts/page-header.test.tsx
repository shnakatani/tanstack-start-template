...
/**
 * 状態のカタログは `page-header.stories.tsx` が持つ (ADR-0039)。ここに残すのは、title が h1 で
 * あること、actions の有無で領域が出入りすること、帯の寸法で、story に play が無い以上ここで
 * しか固定できない。とくに「出ない」ことは見た目のカタログでは表せない (ADR-0039 の役割分担)。
 *
...
describe("PageHeader", () => {
  ...
  it("actions を渡さないとアクション領域が表示されない", async () => {
    ...
    // 肯定 anchor。描画が済んでいることを先に固定してから不在を見る (ADR-0037)
