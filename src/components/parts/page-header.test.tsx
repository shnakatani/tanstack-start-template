...
/**
 * 状態のカタログは `page-header.stories.tsx` が持つ (ADR-0053)。ここに残すのは、title が h1 で
 * あること、actions の有無で領域が出入りすること、帯の寸法で、story に play が無い以上ここで
 * しか固定できない。とくに「出ない」ことは見た目のカタログでは表せない (ADR-0053 の役割分担)。
 *
 * カードのページ見出しとの寸法一致は測らない。どちらも同じ `pageTitle` (`page-title.tsx`) を
 * 当てる 1 つの出処で、外見の上書きは層の規則が止める (ADR-0014)。
 */
describe("PageHeader", () => {
  ...
  it("actions を渡さないとアクション領域が表示されない", async () => {
    ...
    // 肯定 anchor。描画が済んでいることを先に固定してから不在を見る (ADR-0049)
